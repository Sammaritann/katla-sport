...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        {
            var dbHivesSection = await _context.Sections.Where(p => p.Code == updateRequest.Code && p.Id != hiveId).ToArrayAsync();
            if (dbHivesSection.Length > 0)
            {
                throw new RequestedResourceHasConflictException("code");
            }

            dbHivesSection = await _context.Sections.Where(p => p.Id == hiveId).ToArrayAsync();
            if (dbHivesSection.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
        ...
        {
            var dbHivesSection = await _context.Sections.Where(p => p.Id == hiveId).ToArrayAsync();
            if (dbHivesSection.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
